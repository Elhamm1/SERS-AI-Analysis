...
"""
VERIFIED FINAL VERSION — random 80/20 spectrum split.

- Fixed optimized preprocessing and model hyperparameters are used for LR, PLSR, RFR, SVR, and CNN.
- The final 20% test subset is never used for hyperparameter or CNN checkpoint selection.
- The CNN is fitted for at most 100 epochs.
- The exact checkpoint with the lowest internal-validation loss is retained.
- The CNN is not refitted after checkpoint selection.
"""
...
# Fixed values obtained from the completed preprocessing optimization.
...
OPTIMIZED_PLS_COMPONENTS = 15
OPTIMIZED_RFR_PARAMS = {
    "n_estimators": 300,
    "max_depth": 20,
    "max_features": "sqrt",
    "min_samples_leaf": 1,
}
OPTIMIZED_SVR_PARAMS = {
    "C": 1.0,
    "epsilon": 0.01,
    "gamma": "scale",
}

# Iterative modified-polynomial settings used during optimization.
BASELINE_KMAD = 2.5
...
# Location-specific HPLC cysteine concentration (g/100 g).
CULTIVAR_LOCATION_CYS = {
    "AAC_Chrome":     {"01": 0.3105, "02": 0.3312, "03": 0.3100},
    "AAC_Lacombe":    {"01": 0.3712, "02": 0.3145, "03": 0.3300},
    "AAC_Liscard":    {"01": 0.3606, "02": 0.2756, "03": 0.3400},
    "CDC_Amarillo":   {"01": 0.4158, "02": 0.3510, "03": 0.3100},
    "CDC_Athabasca":  {"01": 0.3570, "02": 0.3266, "03": 0.3400},
    "CDC_Canary":     {"01": 0.3181, "02": 0.3053, "03": 0.3200},
    "CDC_Dakota":     {"01": 0.3599, "02": 0.2986, "03": 0.3400},
    "CDC_Golden":     {"01": 0.3236, "02": 0.3836, "03": 0.3700},
    "CDC_Greenwater": {"01": 0.3506, "02": 0.3385, "03": 0.3500},
    "CDC_Inca":       {"01": 0.3918, "02": 0.3542, "03": 0.3500},
    "CDC_Jasper":     {"01": 0.3290, "02": 0.3430, "03": 0.3400},
    "CDC_Striker":    {"01": 0.3446, "02": 0.3703, "03": 0.3200},
    "CDC_Lewochko":   {"01": 0.3850, "02": 0.3088, "03": 0.3300},
    "CDC_Meadow":     {"01": 0.3250, "02": 0.3110, "03": 0.3000},
    "CDC_Patrick":    {"01": 0.3302, "02": 0.3950, "03": 0.2900},
    "CDC_Saffron":    {"01": 0.3326, "02": 0.3778, "03": 0.3300},
    "CDC_Spectrum":   {"01": 0.4142, "02": 0.3753, "03": 0.3300},
    "CDC_Spruce":     {"01": 0.3516, "02": 0.3555, "03": 0.3300},
    "CDC_Tetris":     {"01": 0.3507, "02": 0.3554, "03": 0.3200},
    "Redbat88":       {"01": 0.2965, "02": 0.3423, "03": 0.3100},
}
...
def load_all_spectra(label_mode: str):
    ...
    for file_path in file_paths:
        ...
        if label_mode == "cultivar_mean":
            target = float(CULTIVAR_MEAN_CYS[cultivar])
        elif label_mode == "location_specific":
            target = float(CULTIVAR_LOCATION_CYS[cultivar][location])
        else:
...
def apply_baseline_correction(X, degree):
    ...
    for index in range(len(X)):
        baseline = iterative_modified_polynomial_baseline(
            X[index],
            degree=degree,
        )
        corrected[index] = (
            X[index].astype(np.float64) - baseline
        ).astype(np.float32)
    return corrected
...
def preprocess_for_model(X, model_name):
    ...
    if not np.all(np.isfinite(processed)):
        raise FloatingPointError(
            f"Non-finite values produced for {model_name}"
        )
...
def fit_lr(X_dev, y_dev):
    ...
    return model, "No model hyperparameter selection; optimized LR preprocessing used"


def select_plsr(X_train, y_train, X_val, y_val):
    max_allowed = max(1, min(X_train.shape[0] - 1, X_train.shape[1]))
    candidates = [k for k in PLS_COMPONENTS if k <= max_allowed]
    if not candidates:
        candidates = [1]

    best_rmse, best_k = np.inf, candidates[0]
    for k in candidates:
...
def select_rfr(X_train, y_train, X_val, y_val):
    ...
    for params in RFR_CANDIDATES:
        ...
        rmse = np.sqrt(mean_squared_error(y_val, pred))
        if rmse < best_rmse:
            best_rmse, best_params = rmse, params.copy()
...
def select_svr(X_train, y_train, X_val, y_val):
    best_rmse, best_params = np.inf, None
...
def fit_cnn_with_inner_validation(
    X_train,
    y_train,
    X_val,
    y_val,
    ...
    """Retain the exact checkpoint with the lowest validation loss."""
    set_seed(fold_seed)
    ...
    for epoch in range(1, CNN_MAX_EPOCHS + 1):
        ...
        if patience_count >= CNN_EARLY_STOPPING_PATIENCE:
            break
    ...
    return model, device, best_epoch, best_val, history_df
...
def run_all_models_for_fold(
    ...
    # LR: fixed optimized preprocessing.
    start = time.time()
    X_train_lr = preprocess_for_model(X_train, "LR")
    X_val_lr = preprocess_for_model(X_val, "LR")
    X_dev_lr = preprocess_for_model(X_dev, "LR")
    X_test_lr = preprocess_for_model(X_test, "LR")

    validation_lr = LinearRegression()
    validation_lr.fit(X_train_lr, y_train)
    validation_pred = validation_lr.predict(X_val_lr)
    ...
    selection_rows.append({
        ...
        "Selected_Parameters": (
            f"{lr_note}; "
            f"preprocessing={OPTIMIZED_PREPROCESSING['LR']}"
        ),
    ...
    # PLSR: fixed optimized preprocessing and n_components.
    start = time.time()
    X_train_plsr = preprocess_for_model(X_train, "PLSR")
    X_val_plsr = preprocess_for_model(X_val, "PLSR")
    X_dev_plsr = preprocess_for_model(X_dev, "PLSR")
    X_test_plsr = preprocess_for_model(X_test, "PLSR")

    validation_model = PLSRegression(
        n_components=OPTIMIZED_PLS_COMPONENTS,
        scale=False,
        max_iter=1000,
    )
    validation_model.fit(X_train_plsr, y_train)
    validation_pred = validation_model.predict(X_val_plsr).ravel()
    validation_rmse = np.sqrt(mean_squared_error(y_val, validation_pred))
    ...
    selection_rows.append({
        ...
        "Model": "PLSR",
        "Selected_Parameters": (
            f"n_components={OPTIMIZED_PLS_COMPONENTS}; "
            f"preprocessing={OPTIMIZED_PREPROCESSING['PLSR']}"
        ),
        "Validation_RMSE": validation_rmse,
        "Runtime_Seconds": time.time() - start,
    })
    ...
    # RFR: fixed optimized preprocessing and model parameters.
    start = time.time()
    X_train_rfr = preprocess_for_model(X_train, "RFR")
    X_val_rfr = preprocess_for_model(X_val, "RFR")
    ...
    validation_model.fit(X_train_rfr, y_train)
    validation_pred = validation_model.predict(X_val_rfr)
    validation_rmse = np.sqrt(mean_squared_error(y_val, validation_pred))
    ...
    selection_rows.append({
        ...
        "Model": "RFR",
        "Selected_Parameters": (
            f"{OPTIMIZED_RFR_PARAMS}; "
            f"preprocessing={OPTIMIZED_PREPROCESSING['RFR']}"
        ),
        "Validation_RMSE": validation_rmse,
        "Runtime_Seconds": time.time() - start,
    })
    ...
    # SVR: fixed optimized preprocessing and model parameters.
    start = time.time()
    X_train_svr = preprocess_for_model(X_train, "SVR")
    X_val_svr = preprocess_for_model(X_val, "SVR")
    X_dev_svr = preprocess_for_model(X_dev, "SVR")
    X_test_svr = preprocess_for_model(X_test, "SVR")

    validation_model = SVR(
        kernel="rbf",
        cache_size=2000,
        **OPTIMIZED_SVR_PARAMS,
    )
    validation_model.fit(X_train_svr, y_train)
    validation_pred = validation_model.predict(X_val_svr)
    validation_rmse = np.sqrt(mean_squared_error(y_val, validation_pred))
    ...
    selection_rows.append({
        ...
        "Model": "SVR",
        "Selected_Parameters": (
            f"{OPTIMIZED_SVR_PARAMS}; "
            f"preprocessing={OPTIMIZED_PREPROCESSING['SVR']}"
        ),
        "Validation_RMSE": validation_rmse,
        "Runtime_Seconds": time.time() - start,
    })
    ...
    # CNN: fixed optimized preprocessing; all original CNN training logic remains.
    start = time.time()
    X_train_cnn = preprocess_for_model(X_train, "CNN")
    X_val_cnn = preprocess_for_model(X_val, "CNN")
    X_test_cnn = preprocess_for_model(X_test, "CNN")

    checkpoint_path = fold_output_dir / "best_cnn_validation_checkpoint.pth"
    ...
    history.to_csv(
        fold_output_dir / "cnn_training_history.csv",
        index=False,
    )
    ...
    selection_rows.append({
        ...
        "Model": "CNN",
        "Selected_Parameters": (
            f"best_validation_epoch={selected_epoch}; "
        ...
        "Validation_SmoothL1": validation_loss,
        "Runtime_Seconds": time.time() - start,
    })
    ...
    predictions.to_csv(
        fold_output_dir / "spectrum_predictions.csv",
        index=False,
    )
    selections.to_csv(
        fold_output_dir / "model_selection.csv",
        index=False,
    )
...
def finalize_results(all_predictions, all_selections, unit_columns, output_dir, strategy_name):
    ...
    with open(output_dir / "README_results.txt", "w", encoding="utf-8") as handle:
        handle.write(
            ...
            "Input handling: fixed optimized preprocessing is used for LR, PLSR, RFR, SVR, and CNN.\n"
            ...
            "The CNN retains the exact checkpoint with the lowest inner-validation loss and is not refitted.\n"
        )
...
def main():
    strategy_name = "Random 80/20 spectrum split"
    output_dir = Path("./Unified_Raw_80_20_Comparison")
    X, y, metadata = load_all_spectra(label_mode="cultivar_mean")
    ...
    dev_idx, test_idx = train_test_split(
        ...
        test_size=0.20,
        random_state=SEED,
        stratify=metadata["cultivar"],
    )
    train_idx, val_idx = train_test_split(
        dev_idx,
        test_size=0.20,
        random_state=SEED + 1,
        stratify=metadata.iloc[dev_idx]["cultivar"],
    )

    fold_dir = output_dir / "Fold_1"
    predictions, selections = run_all_models_for_fold(
